...
CouchDb.put = (url, opt, callback) => {
    CouchDb.request('PUT', url, opt, callback);
};

CouchDb.get = (url, opt, callback) => {
    CouchDb.request('GET', url, opt, callback);
};

CouchDb.head = (url, opt, callback) => {
    CouchDb.request('HEAD', url, opt, callback);
};

CouchDb.delete = (url, opt, callback) => {
    CouchDb.request('DELETE', url, opt, callback);
};

CouchDb.request = (method, url, opt, callback) => {
    request(CouchDb.requestBuilder(method, url, opt), (err, res, body) => {
        let parsed;
...
CouchDb.requestBuilder = (method, url, opt = {}) => {
    const req = {
        method,
        url,
