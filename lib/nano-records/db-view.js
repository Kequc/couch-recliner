...
class DbView {
    ...
    find(keys, params, callback = ()=>{}) {
        ...
        if (!keys)
            ...
            this._findStrict(keys, undefined, _.extend({}, params, extended), callback);
        }
    }

    findOne(keys, params, callback = ()=>{}) {
        // find only the first result from the provided parameters
        this.find(keys, params, (err, list) => {
    ...
    findStrict(keys, values, params, callback = ()=>{}) {
        ...
        if (!keys)
            ...
            this._findStrict(keys, values, params, callback);
    }

    findOneStrict(keys, values, params, callback = ()=>{}) {
        // find only the first result from the provided parameters
        this.findStrict(keys, values, params, (err, list) => {
            const doc = list.doc(0);
            if (!err && !doc)
                callback(Err.missing('doc'));
            else
    ...
    _findStrict(keys, values, params, callback, tries = 0) {
        tries++;
                ...
                if (tries <= 1 && (err.name == "no_db_file" || err.name == "not_found")) {
                    ...
                    this._updateNanoRecordsDesign(name, view, (err) => {
                        if (err)
                            ...
                            this._findStrict(keys, values, params, callback, tries);
                    });
                }
                else
                    callback(err, new List(this.db));
