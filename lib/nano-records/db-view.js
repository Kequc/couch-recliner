...
class DbView {
    ...
    findOnly(keys, values, params, callback) {
        ...
        if (!keys)
            ...
            this._findOnly(keys, values, params, callback);
    }
    ...
    find(keys, params, callback) {
        ...
        if (!keys)
            ...
            this._findOnly(keys, undefined, _.extend({}, params, extended), callback);
        }
    }

    _findOnly(keys, values, params, callback, tries = 0) {
        tries++;
                ...
                if (tries <= 1 && (err.name == "no_db_file" || err.name == "not_found")) {
                    ...
                    this._updateNanoRecordsDesign(name, view, (err) => {
                        if (err)
                            ...
                            this._findOnly(keys, values, params, callback, tries);
                    });
                }
                else
                    callback(err, new List(this.db));
