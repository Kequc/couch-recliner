...
class Db {
    constructor(nano, dbName, designs) {
        ...
        this.doc = new DbDoc.default(this);
        this.view = new DbView.default(this);
        this.show = new DbShow.default(this);
    }
    create(callback = () => { }) {
        this._performCreate(callback);
    }
    _performCreate(callback) {
        this.nano.db.create(this.dbName, Err.default.resultFunc('db', callback));
    }
    destroy(verify, callback = () => { }) {
        if (verify !== "_DESTROY_")
            callback(Err.default.verifyFailed('db'));
        else
            this._performDestroy(callback);
    }
    _performDestroy(callback) {
        this.nano.db.destroy(this.dbName, Err.default.resultFunc('db', callback));
    }
