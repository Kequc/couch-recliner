...
class DbDoc {
    constructor(db) {
        this.db = db;
        this.attachment = new DbDocAttachment.default(this);
    }
    create(body, callback = () => { }) {
        if (!body)
            callback(Err.default.missingParam('doc', "body"));
        else
    ...
    _read(id, callback, tries = 0) {
        ...
        this._performRead(id, (err, result) => {
            if (err) {
                ...
                callback(undefined, new Doc.default(this.db, result)); // document found!
        });
    }
    _performRead(id, callback) {
        this.db.raw.get(id, Err.default.resultFunc('doc', callback));
    }
    ...
    _performWriteAndInstantiateDoc(id, rev, body, callback, tries = 0) {
        ...
        this._performWrite(id, rev, clone, (err, result) => {
            if (err) {
                ...
                callback(undefined, new Doc.default(this.db, clone, result)); // written successfully
        });
    }
    _performWrite(id, rev, body, callback) {
        body['_rev'] = rev;
        this.db.raw.insert(body, id, Err.default.resultFunc('doc', callback));
    }
    destroy(id, callback = () => { }) {
        if (!id)
            callback(Err.default.missingId('doc'));
    ...
    _performDestroy(id, rev, callback) {
        this.db.raw.destroy(id, rev, Err.default.resultFunc('doc', callback));
    }
    head(id, callback = () => { }) {
        if (!id)
            callback(Err.default.missingId('doc'));
    ...
    _performHead(id, callback) {
        ...
        this.db.raw.head(id, (raw, body, result) => {
            let err = Err.default.make('doc', raw);
            if (err)
                callback(err);
            else if (result['etag']) {
                // we have a new rev
                ...
                callback(new Err.default('doc', "missing_rev", "Rev missing from header response."));
        });
    }
}
