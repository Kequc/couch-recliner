...
class DocAttachment {
    ...
    write(name, data, mimeType, callback = () => { }) {
        if (!this.doc.getId())
            callback(Err.default.missingId('doc'));
        else if (!name)
            callback(Err.default.missingParam('attachment', "name"));
        else if (!data)
            callback(Err.default.missingParam('attachment', "data"));
        else if (!mimeType)
            callback(Err.default.missingParam('attachment', "mimeType"));
        else
    ...
    _performWrite(name, data, mimeType, callback) {
        this.doc.db.raw.attachment.insert(this.doc.getId(), name, data, mimeType, { rev: this.doc._latestRev }, Err.default.resultFunc('attachment', callback));
    }
    ...
    _performCreateWriteStream(name, data, mimeType, callback) {
        return this.doc.db.raw.attachment.insert(this.doc.getId(), name, data, mimeType, { rev: this.doc._latestRev }, Err.default.resultFunc('attachment', callback));
    }
    ...
    _performDestroy(name, callback) {
        this.doc.db.raw.attachment.destroy(this.doc.getId(), name, { rev: this.doc._latestRev }, Err.default.resultFunc('attachment', callback));
    }
    list() {
        let attachments = [];
