...
class DbDocAttachment {
    ...
    read(id, name, callback = () => { }) {
        if (!id)
            callback(Err.default.missingId('attachment'));
        else if (!name)
    ...
    _performRead(id, name, callback) {
        this.doc.db.raw.attachment.get(id, name, {}, Err.default.resultFunc('attachment', callback));
    }
    ...
    _performCreateReadStream(id, name, callback) {
        ...
        return this.doc.db.raw.attachment.get(id, name, {}, Err.default.resultFunc('attachment', callback));
    }
    ...
    _performWrite(id, rev, name, data, mimeType, callback) {
        this.doc.db.raw.attachment.insert(id, name, data, mimeType, { rev: rev }, Err.default.resultFunc('attachment', callback));
    }
    ...
    _performDestroy(id, rev, name, callback) {
        this.doc.db.raw.attachment.destroy(id, name, { rev: rev }, Err.default.resultFunc('attachment', callback));
    }
}
