...
function _attachments(body = {}, body2 = {}) {
    const _attachments = Object.assign({}, body._attachments || {}, body2._attachments || {});
...
BodyParser.extend = (body, body2) => {
    const attachments = _attachments(body, body2);
    return Object.assign(artisan({}, _strip(body), _strip(body2)), {
        _attachments: Attachment.forParser(attachments)
    });
};

BodyParser.forHttp = (body) => {
    const attachments = _attachments(body);
    const result = Object.assign({}, body, {
        _attachments: Attachment.forHttp(attachments)
    });
    const follows = attachments.filter(attachment => attachment.isFollows());
    if (follows.length > 0)
...
BodyParser.forDoc = (body) => {
    ...
    return Object.assign({}, body, {
        _attachments: Attachment.forDoc(attachments)
    });
};
