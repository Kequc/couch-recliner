...
DocMeta.writeDoc = (doc, body, callback, tries = 0) => {
    tries++;
        ...
        if (err && tries <= MAX_TRIES && err.name === 'conflict') {
            DocMeta.headDoc(doc, (err) => {
                if (err)
                    ...
                    DocMeta.writeDoc(doc, body, callback, tries);
            });
        }
        else if (err)
            callback(err);
...
DocMeta.updateDoc = (doc, body, callback, tries = 0) => {
    tries++;
    const clone = deepExtend({}, doc.body, body);
    _updateDoc(doc, clone, (err, result) => {
        if (err && tries <= MAX_TRIES && err.name === 'conflict') {
            DocMeta.readDoc(doc, (err) => {
                if (err)
                    callback(err);
                else
                    DocMeta.updateDoc(doc, body, callback, tries);
            });
        }
        else if (err)
            callback(err);
...
function _updateDoc(doc, body, callback) {
...
DocMeta.update = (Model, id, body, callback) => {
    DocMeta.read(Model, id, (err, doc) => {
        if (err)
            ...
            DocMeta.updateDoc(doc, body, (err) => {
                if (err)
                    callback(err);
                else
...
DocMeta.destroyDoc = (doc, callback, tries = 0) => {
    tries++;
