...
DocMeta.updateFixed = (doc, addition, callback, tries = 0) => {
    tries++;
    if (doc.rev === undefined || doc._latestRev !== doc.rev) {
        DocMeta.readFixed(doc, (err) => {
            if (err)
                ...
                DocMeta.updateFixed(doc, addition, callback, tries);
        });
    }
    else {
        const url = Req.urlToFixed(doc);
        const payload = addition.extend(doc.body);
        const rev = doc.rev;
        Http.request(Req.put({ url, payload, rev }), Res.body('doc', (err, body) => {
...
DocMeta.update = (Model, id, addition, callback, tries = 0) => {
    tries++;
        ...
        if (err)
            ...
            Http.request(Req.put({ url, payload, rev }), Res.body('doc', (err, body) => {
                if (err && tries <= Model.MAX_TRIES && err.name === 'conflict')
                    DocMeta.update(Model, id, addition, callback, tries);
                else if (err)
                    callback(err);
                else {
                    DocMutator.update(doc, payload, body.rev, body.rev);
