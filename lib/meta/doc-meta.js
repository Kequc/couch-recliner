...
DocMeta.headFixed = (doc, callback) => {
    ...
    Http.request(Req.head({ url }), Res.headers('doc', (err, headers) => {
        if (err && err.name == 'not_found') {
            ...
            doc._latestRev = Res.findRev(headers);
            callback(undefined, doc._latestRev); // success
        }
    }));
};
...
function _found(Model, results) {
    if (results === undefined || results.docs === undefined) return [];
    return results.docs.map(body => {
        const doc = new Model(BodyParser.forDoc(body));
        doc._latestRev = undefined;
        return doc;
    });
}
...
DocMeta.findOne = (Model, finder, callback) => {
    ...
    const body = Object.assign(finder.toQs(), { limit: 1 });
    Http.request(Req.post({ url, body }), Res.body('doc', (err, results) => {
        const doc = _found(Model, results)[0];
        if (err && err.name !== 'not_found')
            callback(err);
...
DocMeta.find = (Model, finder, callback) => {
    const url = Req.urlTo(Model, '_find');
    const body = finder.toQs();
    Http.request(Req.post({ url, body }), Res.body('doc', (err, results) => {
        if (err && err.name !== 'not_found')
            callback(err);
        else
            callback(undefined, _found(Model, results));
    }));
};
