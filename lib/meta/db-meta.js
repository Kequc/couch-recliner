...
DbMeta.head = (Model, callback) => {
    ...
    needle.request('HEAD', url, {}, { json: true }, Res.err('db', callback));
};
...
DbMeta.destroy = (Model, callback) => {
    const url = Model.db.urlTo();
    needle.request('DELETE', url, {}, { json: true }, Res.err('db', callback));
};

DbMeta.create = (Model, callback) => {
    const url = Model.db.urlTo();
    needle.request('PUT', url, {}, { json: true }, Res.err('db', callback));
};
