...
AttachmentMeta.read = (Model, id, attname, callback) => {
    const options = {
        method: 'GET',
        url: Http.urlTo(Model, id, attname)
    };
...
AttachmentMeta.writeFixed = (doc, attname, data, callback, tries = 0) => {
    ...
    const options = {
        method: 'PUT',
        url: Http.urlToFixed(doc, attname),
    ...
    Http.request(options, Res.body('doc', (err, body) => {
        if (err && err.name === 'no_db_file')
            ...
            DocMeta.headFixedAndRetry(doc, callback)(AttachmentMeta.writeFixed, doc, attname, data, callback, tries);
        else if (err)
            callback(err);
        else {
...
AttachmentMeta.destroyFixed = (doc, attname, callback, tries = 0) => {
    ...
    Http.request(options, Res.body('doc', (err, body) => {
        if (err && err.name === 'no_db_file')
            ...
            DocMeta.headFixedAndRetry(doc, callback)(AttachmentMeta.destroyFixed, doc, attname, callback, tries);
        else if (err)
            callback(err);
        else {
