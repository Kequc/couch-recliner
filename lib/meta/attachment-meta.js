...
AttachmentMeta.writeFixed = (doc, attname, attachment, callback, tries = 0) => {
    tries++;
        ...
        if (err && tries <= doc.constructor.MAX_TRIES && err.name === 'conflict') {
            DocMeta.headFixed(doc, (err) => {
                if (err)
                    ...
                    AttachmentMeta.writeFixed(doc, attname, attachment, callback, tries);
            });
        }
        else if (err)
            callback(err);
...
AttachmentMeta.write = (Model, id, attname, attachment, callback, tries = 0) => {
    tries++;
        ...
        if (err)
            ...
            Http.request(Req.put({ url, body, qs: { rev } }), Res.err('doc', (err) => {
                if (err && tries <= Model.MAX_TRIES && err.name === 'conflict')
                    AttachmentMeta.write(Model, id, attname, attachment, callback, tries);
                else if (err)
                    callback(err);
                else
                    callback(); // success
