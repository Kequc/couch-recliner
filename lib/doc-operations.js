...
function _performRead(Model, id, callback) {
    Model.db.get(id, Err.resultFunc('doc', callback));
}
...
function _headDoc(doc, callback) {
    DocOperations.head(doc.constructor, doc.getId(), (err, rev, result) => {
        if (rev)
            doc._latestRev = rev;
...
function _performHead(Model, id, callback) {
    ...
    Model.db.head(id, (raw, body, result) => {
...
DocOperations.create = (Model, body, callback = ()=>{}) => {
    if (!Model)
        ...
        callback(Err.missingParam('body'));
    else
...
function _performWrite(Model, id, rev, clone, callback) {
    clone['_rev'] = rev;
    Model.db.insert(clone, id, Err.resultFunc('doc', callback));
}
...
function _performDestroy(Model, id, rev, callback) {
    Model.db.destroy(id, rev, Err.resultFunc('doc', callback));
}
